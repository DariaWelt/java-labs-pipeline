...
public class PipelineManager implements Runnable {
    private IConsumer pipelineStart;
    ...
    private  RC MakePipeLineDependences(IPipelineStep[] pipline) {
        ...
        try {
            for (int i = 0; i < pipline.length; ++i) {
                if (i == 0)
                    error = (pipline[i]).setProducer(null);
                else
                    error = (pipline[i]).setProducer((IProducer) pipline[i - 1]);
                if (error != RC.CODE_SUCCESS)
                    return error;
                if (i < pipline.length - 1)
                    error = (pipline[i]).setConsumer((IConsumer)pipline[i + 1]);
                if (error != RC.CODE_SUCCESS)
                    return error;
            }
            pipelineStart = (IConsumer)pipline[0];
    ...
    private void BuildPipeline(ManagerParser info) {
        ...
        IPipelineStep[] pipeline = new IPipelineStep[pipsNum];
